#!/usr/bin/env python3
"""Replay shared trigger snapshots through the Java backtest boundary."""
...
def load_snapshots(path: Path) -> list[MarketDataSnapshot]:
...
def run(input_path: Path, output_path: Path) -> None:
    ...
    try:
        ...
        engine.submit_stop_market_order(
            "trigger", "AAPL", "stop_market_bid_ask", "BUY", 1, 100, 105.0, "BID_ASK"
        )
        engine.submit_stop_market_order(
            "trigger", "AAPL", "stop_market_last", "BUY", 1, 100, 105.0, "LAST_PRICE"
        )
        engine.submit_stop_limit_order(
            "trigger", "AAPL", "stop_limit_last", "BUY", 1, 100, 104.0, 105.0, "LAST_PRICE"
        )
        rows: list[dict[str, object]] = []
        previous: dict[str, str] = {}
        for snapshot in snapshots[:1]:
            for order_id in ("stop_market_bid_ask", "stop_market_last", "stop_limit_last"):
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=Path(__file__).parent / "output" / "java_trigger_states.csv")
